...
ip.isPrivate = function (addr) {
  ...
  // SECURITY (GHSA-2p57-rm9w-gvfp): 0.0.0.0/8 is reserved (RFC 1122 "this
  // network") and on most kernels routes to localhost — leaving it out of
  // isPrivate is what makes the upstream `ip` package SSRF-vulnerable.
  // Same risk for the 100.64.0.0/10 carrier-grade NAT range. Both added
  // here so consumers using isPublic() as an allow-list don't leak.
  return /^(::f{4}:)?0\.([0-9]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})$/i.test(addr)
    || /^(::f{4}:)?10\.([0-9]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})$/i.test(addr)
    || /^(::f{4}:)?100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\.([0-9]{1,3})\.([0-9]{1,3})$/i
      .test(addr)
