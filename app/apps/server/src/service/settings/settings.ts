...
import type { Setting, SettingsTable, UpsertSettingInput, OperationResult } from "./types";
...
function log(level: "debug" | "info" | "warning" | "error", message: string) {
  ...
  console.log(`[${level.toUpperCase()}] [SETTINGS_SERVICE] ${message}`);
}
 ...
 */
export function deleteSetting(table: SettingsTable, key: string): OperationResult {
  try {
    ...
    const checkQuery = db.query(`SELECT COUNT(*) as count FROM ${table} WHERE key = ?`);
