...
// Server manages queue state only. Audio playback state comes from Tauri.
...
// Called when Tauri sends audio state updates - this is the source of truth
...
async function playNext(): Promise<void> {
  ...
  if (nextIndex < queueLength) {
    ...
    // End of queue - just update queue state, let Tauri handle playback state
    updateState({
      currentIndex: -1,
      currentTrack: null,
    })
...
async function playAtIndex(index: number): Promise<void> {
  ...
  // Only update queue-related state, not playback state
  updateState({
    currentIndex: index,
    currentTrack,
  ...
  // Send load command to Tauri - it will respond with actual playback state
  ...
  switch (command.type) {
    case 'play':
      if (playerState.currentIndex === -1 && getQueueLength() > 0) {
        ...
        // Just send to Tauri - it will respond with state update
