...
/**
 * Gets a search history entry by URL path
 */
export function getSearchByUrlPath(
  urlPath: string,
): { data: SearchHistoryItem | null } | { error: string } {
  try {
    log('debug', `Getting search history for URL: ${urlPath}`)
    ...
    if (!result) {
      log('debug', `No search history found for URL: ${urlPath}`)
      return { data: null }
    }
