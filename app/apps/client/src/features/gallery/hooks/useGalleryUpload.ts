 ...
 * are not sent in parallel. Heavy animated GIFs among them first need the
 * warning (`heavyGifWarning`, to render) confirmed, or nothing is uploaded.
 * Each refused or failed file gets its own error toast and the rest carry on;
 * a summary toast reports how many made it. The list refreshes after every
 * upload through the upload hook's invalidation.
 */
export function useGalleryUpload() {
  ...
  return { uploadFiles, progress, heavyGifWarning }
}
