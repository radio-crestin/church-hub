...
/**
 * Migrate old position/size format to new constraints format
 */
export function migratePositionToConstraints(
  position: OldPosition,
  size: OldSize,
): { constraints: Constraints; size: SizeWithUnits } {
  return {
    constraints: {
      top: constraint(true, position.y, position.unit),
      bottom: constraint(false, 0, '%'),
      left: constraint(true, position.x, position.unit),
      right: constraint(false, 0, '%'),
    },
    size: {
      width: size.width,
      widthUnit: size.unit,
      height: size.height,
      heightUnit: size.unit,
    },
  }
}

/**
 * Check if an element config uses the old position format
 */
export function needsMigration(
  config: unknown,
): config is { position: OldPosition; size: OldSize } {
  return (
...
/**
 * Migrate a single element config from old format to new format
 */
export function migrateElementConfig<T extends object>(config: T): T {
  if (!needsMigration(config)) {
    return config
  }

  const { position, size, ...rest } = config
  const migrated = migratePositionToConstraints(position, size)
...
/**
 * Migrate an entire content config (song, bible, etc.)
 */
export function migrateContentConfig<T extends object>(config: T): T {
  const result = { ...config }

  for (const key of Object.keys(result) as (keyof T)[]) {
    const value = result[key]
    if (typeof value === 'object' && value !== null && needsMigration(value)) {
      result[key] = migrateElementConfig(value) as T[keyof T]
    }
  }
